...
void MenuModule::Draw() {
    ...
    DrawText(
        "Play",
        static_cast<int>(playButton.x + ((playButton.width - MeasureText("Play", scaledFontSize)) / 2.0f)),
        static_cast<int>(playButton.y + (playButton.height - scaledFontSize) / 2.0f),
        scaledFontSize,
        BLACK
    );
    ...
    DrawText(
        ...
        static_cast<int>(settingsButton.y + (settingsButton.height - scaledFontSize) / 2.0f),
        scaledFontSize,
        BLACK
    );
}
 ...
 * set the original action gets set to none so it doesn't run every frame.
 * original(Play) -> copy(Play) -> original(None) return copy(Play)
 */
